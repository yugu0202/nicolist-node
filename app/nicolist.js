...
/*
query = {
  text: "SELECT id,parents_id,child_count,message,message_time FROM messages WHERE parents_id IS NULL",
};
...
async function MylistAdd(name,page)
{
  ...
  await page.waitForSelector(`*[data-mylist-name='${name}']`);
  await page.screenshot({path: 'ml.png'});
...
async function DBCheck(mylistName)
{
  console.log("DBcheck start");
  
  query = {
    text: "SELECT id FROM mylist WHERE name == $1 VALUES ($1,)",
    values: [mylistName],
  };
